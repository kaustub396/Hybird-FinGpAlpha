"""
Cross-Market Generalisation — Phase 2: Feature Engineering for S&P 500
Mirrors build_features.py exactly — same 32 features, same logic.

Reads  : data/raw_sp500/*.csv
Outputs: data/processed_sp500/features/{STOCK}.csv
         data/processed_sp500/panel_sp500.pkl  (main input for GP engine)
"""
...
PROC_DIR      = os.path.join(BASE_DIR, "data", "processed_sp500")
...
stock_files = sorted([
    ...
    if f.endswith('.csv') and f != 'SP500_INDEX.csv'
])
print(f"Loading {len(stock_files)} S&P 500 stocks from {RAW_DIR}...")
...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Feature set is market-agnostic — no India-specific inputs.
    """
    df = df.copy()

    # Normalise column names (yfinance may return Price/Adj Close variants)
...
# Require at least 30 stocks to have data on a given day
valid_dates = date_counts.index[date_counts.notna().sum(axis=1) >= 30]
print(f"  Valid dates (>=30 stocks): {len(valid_dates)}  "
...
# Save panel
panel_path = os.path.join(PROC_DIR, "panel_sp500.pkl")
pd.to_pickle(panel, panel_path)

print(f"\n{'=' * 65}")
print("  S&P 500 Feature Engineering Complete!")
print(f"{'=' * 65}")
